...
for s in [0.5,1,1.5,2]:
    ...
    for n in range(6):
        def ode_func(xi, arg):
            ...
            # if lam<1e-5: v=-v
            try:
                return (v, -2/9 * M(lam)/lam**2 - de*(de-1)*lam - (2*de-1)*v + 1e-50/lam**30)
            except:
                print(lam,s, v, xi)
                raise Exception
        ...
        # M_vals += quad(lambda xi : np.exp(-xi*2*s/3), xi[-1], np.inf)[0]
        ...
        if n in [0,1,3,5]:
            ...
            ax4.plot(xi,lam, color=color_this, label=f's={s}, n={n}', ls=ls)
            ...
            lam = np.linspace(0,1)
            ax5.plot(lam, M(lam)/M(1), color=color_this, label=f's={s}, n={n}', ls=ls)
            # ax5.plot(lam, M0(lam), color=color_this)
